...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($action === 'login') {
        ...
        if (!$identifier || !$password) {
            ...
            $stmt = $db->prepare("SELECT * FROM users WHERE student_id = ? OR email = ?");
    ...
    elseif ($action === 'register') {
        ...
        if (!$firstName || !$lastName || !$studentId || !$course || !$courseLevel || !$email || !$address || !$password) {
            ...
            if ($dup->fetch()) {
                ...
                $db->prepare("
                    INSERT INTO users (first_name,last_name,middle_name,student_id,course,course_level,email,address,password)
                    VALUES (?,?,?,?,?,?,?,?,?)
                ")->execute([$firstName,$lastName,$middleName,$studentId,$course,$courseLevel,$email,$address,password_hash($password,PASSWORD_DEFAULT)]);
                $activeTab = 'login';
      ...
      <div class="auth-panel <?= $activeTab === 'login' ? 'active' : '' ?>" id="panel-login">
        ...
        <form method="POST" action="">
          ...
          <div class="form-group">
            ...
            <input type="text" name="username" placeholder="e.g. 22-12345 or you@uc.edu.ph" autocomplete="username" required />
          </div>
          <div class="form-group">
            <label>Password</label>
            <div class="input-wrapper">
              <input type="password" name="password" placeholder="Enter your password" autocomplete="current-password" required />
              <span class="input-icon toggle-pass"><i class="bi bi-eye"></i></span>
            </div>
          </div>
          ...
          <p class="switch-auth">Don't have an account? <a data-switch="register">Create one</a></p>
        </form>
      </div>
      ...
      <div class="auth-panel <?= $activeTab === 'register' ? 'active' : '' ?>" id="panel-register">
        ...
        <form method="POST" action="">
          ...
          <div class="form-group">
            ...
            <input type="text" name="student_id" placeholder="e.g. 22-12345" required />
          </div>
          <div class="form-row">
            <div class="form-group">
              <label>Course *</label>
          ...
          <div class="form-group">
            ...
            <div class="input-wrapper">
              <input type="password" name="confirm_password" placeholder="Re-enter your password" autocomplete="new-password" required />
              <span class="input-icon toggle-pass"><i class="bi bi-eye"></i></span>
            </div>
          </div>
